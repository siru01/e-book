...
def search(query: str, page: int = 1):
    ...
    resp = requests.get(
        ...
        params={"q": query, "page": page, "limit": 20,
                "fields": "key,title,author_name,cover_i,subject,first_publish_year,first_sentence"},
        timeout=8
    )
...
def by_category(genre: str, page: int = 1):
    ...
    for w in works:
        ...
        results.append({
            ...
            "source":         "openlibrary",
            "read_url":       f"{BASE_URL}{ol_key}",
            "subjects":       [s.get("name", "") for s in w.get("subject", [])[:5]],
...
def new_arrivals():
    ...
    if cached:
        return cached
    results = search("new releases", page=1)
